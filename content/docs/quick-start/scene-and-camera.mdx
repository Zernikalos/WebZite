...
<Tabs items={['Kotlin', 'JavaScript', 'Swift']} groupId="lang">
  <Tab value="Kotlin">
    ...
    camera.lens.setVerticalFovDegrees(60f)
  ...
  <Tab value="JavaScript">
    ...
    camera.lens.setVerticalFovDegrees(60);
    scene.addChild(camera);
...
On **iOS**, if you manage the camera yourself, call `camera.setViewportResize(width:height:)` from `onResize` so the projection matches the drawable size. The engine also propagates viewport size through `ZObject.onViewportResize`.
